...
async function requestDemoToken() {
  ...
  try {
    ...
    if (result.available && result.token) {
      ...
      status.textContent = 'Form opened in new tab. Or self-host: clone github.com/AUTHENSOR/AUTHENSOR and run docker compose up -d';
      var help = document.getElementById('demo-token-help');
      if (help) help.textContent = 'Fill out the form, check your email for the token, then paste it below. Or self-host the control plane: clone github.com/AUTHENSOR/AUTHENSOR and run docker compose up -d (http://localhost:3000).';
    }
  } catch (err) {
